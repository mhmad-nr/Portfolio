...
const MyExperince = forwardRef<HTMLDivElement, props>(({ position }, ref) => {
    ...
    data: [
      {
        ...
        discription:
          "a website for karsin that includes  many other images and images ",
        gitHub: undefined,
        texture: null,
      },
      {
  ...
  const changeTexture = (obj: any, isReverse: boolean = false) => {
  ...
  const desktopAnimation = (isReverse: boolean) => {
    ...
    const obj: any = engine.scene.children.filter((item) => {
      if ((item as any).geometry instanceof THREE.PlaneGeometry) {
        console.log(item);
        return item;
      }
    })[0];

    gsap.to((obj as any).material.uniforms.dispPower, 0.5, {
      value: 1,
      ease: Expo.easeInOut,
  ...
  const init = async () => {
    ...
    const textures = data.map(
      ({ texture }: { texture: THREE.Texture }) => texture
    );
    ...
    const mat = new THREE.ShaderMaterial({
      uniforms: {
        dispPower: { type: "f", value: 0.0 } as any,
        intensity: { type: "f", value: 0.5 } as any,
        res: { value: new THREE.Vector2(width, height) },
        size: { value: new THREE.Vector2(1, 1) },
        texture1: { type: "t", value: textures[0] } as any,
        texture2: { type: "t", value: textures[1] } as any,
        disp: { type: "t", value: disp.texture } as any,
      },
      transparent: true,
      vertexShader: vert,
      fragmentShader: frag,
    ...
    const geometry = new THREE.PlaneGeometry(plane.width, plane.height, 1);
              ...
              <SwiperSlide key={name}>
                <div className="w-full pt-[15rem] h-screen flex items-center justify-center">
                  <div className="p-4 rounded-lg galss-container">
                    <h2 className="text-2xl font-bold text-[#4c4c4c] capitalize">
                      {name}
                    </h2>
                    <div className="w-96">
                    ...
                    <a
                      className="block text-blue-600 font-bold text-base capitalize my-2"
                      href={url}
                    >
                      visit
                    </a>
                    {gitHub && (
                      <a
                        className="block text-blue-600 font-bold text-base capitalize"
                        href={gitHub}
                      >
                        see source
                      </a>
